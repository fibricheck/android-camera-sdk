...
public class SGFilter {
  ...
  public void fillSgData() {
    ...
    Arrays.fill(sgData, 0.0);
  }

  public double applyFilter() {

    // Values calculated from http://www.statistics4u.com/fundstat_eng/cc_savgol_coeff.html
    return switch (sgData.length) {
      case 5 ->
          (((-3) * sgData[0]) + (12 * sgData[1]) + (17 * sgData[2]) + (12 * sgData[3]) - (3 * sgData[4])) / (double) 35;
      case 7 ->
          (((-2) * sgData[0]) + (3 * sgData[1]) + (6 * sgData[2]) + (7 * sgData[3]) + (6 * sgData[4]) + (3 * sgData[5]) - (2 * sgData[6]))
              / (double) 21;
      case 9 -> (((-21) * sgData[0])
          + (14 * sgData[1])
          + (39 * sgData[2])
