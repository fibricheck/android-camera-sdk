...
class ProcessMeasurementWorker(
  ...
  val measurementRawList: MutableList<MeasurementRaw>,
  val addGyro: Boolean,
  val addAcc: Boolean,
  val addRotation: Boolean,
  val addGrav: Boolean
) {
  var cancelled = false

  fun execute(onFinished: (data: MeasurementData) -> Unit, onCancelled: () -> Unit = {}) {
  ...
  private suspend fun process(): MeasurementData {
